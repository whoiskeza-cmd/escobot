...
Total Profit: ${s.get('profit', 0.0):.2f}"""
    ...
    if data in ["format", "sale", "replace", "tester"]:
        ...
        if data == "sale":
            ...
            await query.edit_message_text("Send cards or drop .txt file to continue.", reply_markup=pre_buttons())
        return
...
async def process_check(query, session, uid, context):
    ...
    post_text += f"Live Rate: 100.00%\n\n"
    post_text += f"Mode: {mode}"
...
async def send_formatted_file(query, session, uid, context):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write(content)
    # Fixed file sending - use context.bot
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if session.get("awaiting_filename"):
        ...
        return
    # Flow handling for Sale, Replace, Tester
    ...
    if session.get("mode") == "tester" and not session.get("tester_type"):
        ...
        return
    # Parse cards - supports your new format
    lines = []
...
def main():
    if not TOKEN:
        ...
        return
    print("FactoryVHQ Bot Started - FULL TEST MODE (Owner ID Restored - File Sending Fixed)")
